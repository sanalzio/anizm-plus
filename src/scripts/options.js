...
customThemeInput.addEventListener("change", () => {
    switchTheme(getThemeSelectValue(themeSelect.value));
});
...
function hexToHsl(value) {
    ...
    const rHex = parseInt(value.slice(0,2), 16);
    const gHex = parseInt(value.slice(2,4), 16);
    const bHex = parseInt(value.slice(4,6), 16);
...
function getThemeSelectValue(value) {
    if (value == "custom") {
        const hex = customThemeInput.value.trim().startsWith("#")
                     ? customThemeInput.value.trim().slice(1, 7)
                     : customThemeInput.value.trim().slice(0, 6)
        const hsl = hexToHsl(hex);
        return `$${hex} ${hsl.h} ${hsl.s} ${hsl.l}`; // ${hsl.l} ${hsl.s / 61} ${((hsl.l / 34) > 1 ? 1 + ((hsl.l / 34) - 1) * 1.7 : hsl.l / 34)}
    }
    return value;
}
...
for (let i = 0; i < bgSelContainers.length; i++) {
    ...
    const colorInputElementDefault = colorInputElement.value || "#000000";
    ...
    bgSel.addEventListener("change", () => {
        switch (bgSel.value) {
            ...
            case "color":
                ...
                colorInputElement.value = colorInputElementDefault;
...
function applySettings(result) {
    ...
    if (result.themeId) {
        if (result.themeId.startsWith("$")) {
            customThemeInput.value = "#" + result.themeId.slice(1, 7);
            themeSelect.value = "custom";
            customThemeInputNewDsiplay = "inline-block";
        } else themeSelect.value = result.themeId;
    } else themeSelect.value = "orange";
...
document.getElementById('fileinput').addEventListener('change', function(event) {
    ...
    reader.onload = function(e) {
        ...
        browserObj.storage.local.get(dataKeys, results=>{
            const newSettings = {...defaultSettings, ...results, ...content};
            browserObj.storage.local.set(newSettings, () => {
                ...
                if (applyColor.checked) switchTheme(getThemeSelectValue(themeSelect.value));
